...
class API(base_api.API):
    ...
    def get_item(self, context, name, zone_id=None):
        ...
        volumes = self._filter_volumes_by_zone(volumes, zone_id)
    ...
    def _filter_volumes_by_zone(self, volumes, zone_id):
        if zone_id is None:
            return volumes
        return filter(lambda volume: volume["availability_zone"] == zone_id,
                      volumes)

    def delete_item(self, context, name, zone_id=None):
        volume = self.get_item(context, name, zone_id)
        self._volume_service.delete(context, volume["id"])
    ...
    def add_item(self, context, name, body, zone_id=None):
        ...
        if snapshot_uri:
            ...
            if image_name:
                image = image_api.API().get_item(context, image_name, zone_id)
                image_id = image['id']
        ...
        return self._volume_service.create(context,
                sizeGb,
                body.get('name'),
