...
class API(base_api.API):
    ...
    def get_item(self, context, name, zone_id=None):
        return self.search_items(context, {"name": name}, zone_id)[0]

    def get_items(self, context, zone_id=None):
        return self.search_items(context, None, zone_id)

    def get_zones(self, context, item):
        return [zone_api.API().get_item_by_host(context, item["host"])]

    def search_items(self, context, search_opts=None,
                     zone_id=None, prepared=True, want_objects=False):
        ...
        for instance in instances:
            if zone_id is None or zone_id in self.get_zones(context, instance):
                if not want_objects:
    ...
    def _can_delete_network(self, context, network):
        instances = self.search_items(context)
        for instance in instances:
    ...
    def _add_secgroup_to_instances(self, context, secgroup, **kwargs):
        ...
        affected_instances = self._get_instances_with_network(
                context, network, kwargs["zone_id"])
    ...
    def reset_instance(self, context, zone_id, name):
        instance = self.search_items(context, {"name": name}, zone_id,
                                     want_objects=True)[0]
    ...
    def add_item(self, context, name, body, zone_id=None):
        ...
        image_link = body.get('image')
        if image_link is not None:
            image_name = image_link.split('/')[-1]
            image_id = image_api.API().get_item(
                context, image_name, zone_id)["id"]
        ...
        for disk in instance_disks:
            ...
            try:
                ...
                if (image_link is None
                    and disk['boot']
                    and 'volume_image_metadata' in volume):
        ...
        instance_type = machine_type_api.API() \
            .get_item(context, flavor_name, zone_id)

        (instances, dummy) = self._compute_api.create(
            context,
            instance_type,
            image_id,
            display_name=name,
    ...
    def add_access_config(self, context,
                          body, item_id, zone_id, network_interface):

        address = body.get('natIP')

        instance = self.search_items(context, {"name": item_id}, zone_id)[0]
    ...
    def delete_access_config(self, context, item_id, zone_id,
