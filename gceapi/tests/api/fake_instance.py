...
def fake_instance_get_all(self, context, search_opts=None):
    if "name" in search_opts:
        name = search_opts["name"]
        return [i for i in FAKE_INSTANCES if i["display_name"] == name]
    if "fixed_ip" in search_opts:
        fip = search_opts["fixed_ip"]
        for i in FAKE_INSTANCES:
            subnets = i["info_cache"]["network_info"][0]["network"]["subnets"]
            fips = subnets[0]["ips"][0]["floating_ips"]
            if fips and fips[0]["address"] == fip:
                return [i]
        return []
